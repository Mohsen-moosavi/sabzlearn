...
exports.create = async (req, res, next) => {
  try {
    const {id, title, description, body, shortName, categoryID } = req.body;
    ...
    const oldArticle = await articleModel.findOne({_id : id})
    if(oldArticle){
      const newArticle = await articleModel.findOneAndUpdate({_id : id} , {$set :{
        title,
        description,
        shortName,
        body,
        ...
        const populatedNewArticle = await newArticle.populate("creator", "-password");

        return res.status(201).json(populatedNewArticle);
    }
...
exports.saveDraft = async (req, res, next) => {
  try {
    ...
    await articleModel.validation({ ...req.body }).catch((err) => {
      err.statusCode = 400;
      throw err;
    });
    ...
    if(oldArticle){
      const newArticle = await articleModel.findOneAndUpdate({_id : id},{$set : {title , description,shortName,body,categoryID ,publish: 0,}})
      const newDraftedArticle = await newArticle.populate("creator", "-password");
      return res.status(200).json(newDraftedArticle);
    }
